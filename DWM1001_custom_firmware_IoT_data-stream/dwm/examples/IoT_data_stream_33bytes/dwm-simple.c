...
#define MSG_INIT	\
	"\n\n"	\
	"App   :  [IoT Data-Stream 33 Bytes]\n"	\
	"Built :  " __DATE__ " " __TIME__ "\n"	\
	"\n"

/* define global variables */
char strIOTanALL[50];
unsigned int distAn[] = {0,0,0,0};
unsigned int idAn[] = {0,0,0,0};
	...
	switch (p_evt->header.id) 
        {
          ...
          case DWM_EVT_LOC_READY:
                  ...
                  */
                  ...
                  /*----------------------------------------------------------------------------------------------------------------------------------------------------*/
                  /* acquire and store desired data*/
	          for (i = 0; i < p_evt->loc.anchors.dist.cnt; ++i) 
                  {
                          idAn[i] = (unsigned int)(p_evt->loc.anchors.dist.addr[i]);

                          distAn[i] = p_evt->loc.anchors.dist.dist[i]/10 ;  // cm

                          totalAnCount = p_evt->loc.anchors.dist.cnt;
                  }

                  /* create [Iot data-stream 33 bytes] */
...
void app_thread_entry(uint32_t data)
{
	...
	APP_ERR_CHECK(dwm_cfg_get(&cfg));
        ...
        /* Update rate set to 100 msecond=10Hz, stationary update rate set to 5 seconds */
	//APP_ERR_CHECK(dwm_upd_rate_set(1, 10));
...
{        
	...
	dwm_shell_compile(); // UART serial ready to response
	/* Disabling ble by default as softdevice prevents debugging with breakpoints (due to priority) */
	dwm_ble_compile(); //--- Bluetooth ready to response
	dwm_le_compile(); // location engine ready to response
	dwm_serial_spi_compile(); // SPI port ready to response
